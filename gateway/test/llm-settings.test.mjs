...
const { normalizeLlmSettings, DEFAULT_SETTINGS } = await import('../src/llm-settings.mjs?test=settings');

test('LLM settings expose safe defaults', () => {
  const value = normalizeLlmSettings();
...
test('LLM settings clamp update interval', () => {
  const value = normalizeLlmSettings({ autoUpdateEnabled: true, intervalHours: 999 });
