...
/**
 * Attach the WSS runtime to the existing Gateway HTTP server.
 * Authentication and business/event delivery remain injectable boundaries.
 * @param {import('node:http').Server} server
 * @param {{
 *   authenticateDevice?: (message: Record<string, unknown>) => boolean | { ok: boolean, reason?: string } | Promise<boolean | { ok: boolean, reason?: string }>,
 *   onMessage?: (message: Record<string, unknown>, session: object) => unknown | Promise<unknown>,
...
export function attachWssTransport(server, options = {}) {
  ...
  async function processMessage(session, message) {
    ...
    if (session.state === STATES.CONNECTING) {
      ...
      if (!auth?.ok) {
        send(session, envelope({
          ...
          payload: { code: auth?.reason ?? 'authentication_failed' },
    ...
    if (message.message_type === 'PING') {
      send(session, envelope({ messageType: 'PONG', deviceId: session.deviceId, payload: message.payload, correlationId: message.correlation_id, causationId: message.message_id }));
      return;
    }

    if (message.message_type === 'RESUME') {
      const last = resumeState.get(session.deviceId);
      const requested = message.payload && typeof message.payload === 'object' ? message.payload.last_sequence : null;
      if (last && (requested === last.lastSequence || requested === null)) {
        session.lastSequence = last.lastSequence;
        send(session, envelope({ messageType: 'RESUME_OK', deviceId: session.deviceId, payload: { last_sequence: session.lastSequence ?? 0 }, causationId: message.message_id }));
      } else {
        send(session, envelope({ messageType: 'STATE_SYNC_REQUIRED', deviceId: session.deviceId, payload: { reason: 'resume_unavailable' }, causationId: message.message_id }));
      }
      return;
    }
    ...
    if (typeof message.sequence === 'number') {
      ...
      if (session.lastSequence !== null && message.sequence > session.lastSequence + 1) {
        send(session, envelope({ messageType: 'STATE_SYNC_REQUIRED', deviceId: session.deviceId, payload: { reason: 'sequence_gap', expected: session.lastSequence + 1, received: message.sequence }, causationId: message.message_id }));
        return;
      }
    ...
    if (message.message_type === 'EVENT') {
      ...
      send(session, envelope({
        messageType: 'ACK',
        deviceId: session.deviceId,
        payload: { event_id: message.event_id },
    ...
    try {
      await onMessage(message, {
        state: session.state,
        deviceId: session.deviceId,
        send: (outbound) => send(session, outbound),
      });
    } catch (error) {
    ...
    socket.on('data', async chunk => {
      ...
      try {
        while (buffer.length) {
          ...
          if (opcode !== 0x1) {
            fail(session, 'unsupported_frame', 'only text frames are supported');
            return;
          }
          let message;
          try {
