...
import { getLlmProviderStatus, updateAllLocalModels } from './llm.mjs';
...
export function createHttpServer({ readinessChecks = {} } = {}) {
  ...
  return createServer(async (request, response) => {
    ...
    if (request.method === 'GET' && url.pathname === '/api/llm/models') return json(response, 200, await getLlmProviderStatus());
    ...
    if (request.method === 'GET' && url.pathname === '/api/credentials') {
      return json(response, 200, { credentials: listCredentialStatus() });
    }
    if (request.method === 'PUT' && url.pathname === '/api/credentials') {
      try {
        const body = await parseBody(request);
        return json(response, 200, setCredential(String(body.key ?? ''), String(body.value ?? '')));
      } catch (error) {
        return json(response, 400, { error: error instanceof Error ? error.message : String(error) });
      }
    }
    if (request.method === 'DELETE' && url.pathname === '/api/credentials') {
      try {
        return json(response, 200, deleteCredential(url.searchParams.get('key') ?? ''));
      } catch (error) {
        return json(response, 400, { error: error instanceof Error ? error.message : String(error) });
      }
    }
    ...
    if (request.method === 'POST' && url.pathname === '/api/whatsapp/connect') {
      ...
      catch (error) { return json(response, 500, { error: error instanceof Error ? error.message : String(error), status: getStatus() }); }
    }
