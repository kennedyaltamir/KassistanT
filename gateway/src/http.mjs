...
/** @typedef {Record<string, any>} RequestBody */
...
function parseBody(request) {
  return new Promise((resolve, reject) => {
    ...
    request.on('end', () => { if (!raw) return resolve({}); try { const parsed = JSON.parse(raw); return resolve(parsed && typeof parsed === 'object' && !Array.isArray(parsed) ? parsed : {}); } catch { reject(new Error('Invalid JSON body')); } });
...
export function createHttpServer({ readinessChecks = {}, dispatchRuntime = defaultBatchDispatchRuntime } = {}) {
  ...
  return createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === 'GET' && url.pathname === '/api/whatsapp/ai/config') return json(response, 200, { ...getAiConfig(), configuredConversations: listConversationPolicies().length });
      if (request.method === 'PUT' && url.pathname === '/api/whatsapp/ai/config') { try { const body = await parseBody(request); const allowed = ['enabled','baseUrl','model','timeoutMs','contextMessages','cooldownMs','systemPrompt']; const patch = Object.fromEntries(Object.entries(body).filter(([key]) => allowed.includes(key))); return json(response, 200, updateAiConfig(patch)); } catch (error) { return json(response, 400, { error: sanitizedError(error) }); } }
      ...
      if (request.method === 'POST' && url.pathname === '/api/credentials/validate') { try { const body = await parseBody(request); return json(response, 200, await validateCredential(String(body.key ?? ''))); } catch (error) { return json(response, 400, { error: sanitizedError(error) }); } }
      if (request.method === 'GET' && url.pathname === '/api/whatsapp/ai/conversations') { const jid = url.searchParams.get('jid'); return jid ? json(response, 200, getConversationPolicyStatus(jid)) : json(response, 200, { policies: listConversationPolicies() }); }
      if (request.method === 'PUT' && url.pathname === '/api/whatsapp/ai/conversations') { try { const body = await parseBody(request); return json(response, 200, setConversationPolicy(String(body.jid ?? ''), body)); } catch (error) { return json(response, 400, { error: sanitizedError(error) }); } }
      if (request.method === 'DELETE' && url.pathname === '/api/whatsapp/ai/conversations') { try { return json(response, 200, clearConversationPolicy(url.searchParams.get('jid') ?? '')); } catch (error) { return json(response, 400, { error: sanitizedError(error) }); } }
      ...
      if (batchMatch) {
        ...
        if (request.method === 'POST') {
          try {
            ...
            if (action === 'confirm') return json(response, 200, { batch: await dispatchRuntime.confirmBatch(batchId, { fingerprint: actionBody.fingerprint, recipientCount: actionBody.recipient_count, correlationId: id, confirmedAt: typeof actionBody.confirmed_at === 'string' ? actionBody.confirmed_at : undefined }) });
      ...
      if (request.method === 'POST' && url.pathname === '/api/whatsapp/messages') { try { const body = await parseBody(request); const message = await sendText(String(body.to ?? ''), String(body.text ?? '')); return json(response, 202, { message }); } catch (error) { const message = sanitizedError(error); return json(response, message.includes('not connected') ? 409 : 400, { error: message }); } }
      return json(response, 404, { error: { code: 'not_found', message: 'Route not found.', retryable: false, correlation_id: id } });
    } catch { return json(response, 500, { error: { code: 'internal_error', message: 'Internal server error.', retryable: true, correlation_id: id } }); }
  });
}
export { connect };
