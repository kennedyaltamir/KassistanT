...
export class CampaignDispatchRuntime {
  ...
  async createDraft(preview, options = {}) {
    ...
    for (const recipient of snapshot.recipients) { const selection = this.#selectForRecipient(batchId, recipient, snapshot, scheduledAt == null ? null : new Date(scheduledAt).getTime()); record.selections[recipient.normalizedNumber] = selection; scheduledAt = selection.scheduledAt; }
