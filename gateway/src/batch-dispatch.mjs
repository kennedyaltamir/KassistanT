...
function defaultStatePath() { return process.env.KASSIST_BATCH_STATE_PATH ?? path.join(defaultDataRoot(), 'dispatch', 'batches.json'); }
/** @param {()=>number} clock */
...
/** @param {unknown} error @returns {{kind:'RETRYABLE'|'TERMINAL',message:string}} */
function classifyFailure(error){
  const message=error instanceof Error?error.message:String(error);
  let statusCode;
  if(typeof error==='object' && error!==null && 'output' in error && typeof error.output==='object' && error.output!==null && 'statusCode' in error.output && typeof error.output.statusCode==='number') statusCode=error.output.statusCode;
  else if(typeof error==='object' && error!==null && 'statusCode' in error && typeof error.statusCode==='number') statusCode=error.statusCode;
...
function retryDelay(attemptNumber){ const delay=BACKOFF_MS[attemptNumber-1]; if(delay === undefined) throw new Error(`No backoff policy for attempt ${attemptNumber}`); return delay; }
function makeInitialState(){ /** @type {DispatchJournal} */ const initial={version:/** @type {1} */ (1),batches:{}}; return initial; }
...
export class BatchDispatchRuntime {
  ...
  #aggregate(batch){
    ...
    if(hasProcessing) return 'PROCESSING'; if(hasRetryWait||hasPending) return 'PROCESSING'; if(hasSuccess&&hasTerminalFailure) return 'PARTIAL_FAILURE'; if(recipients.length>0&&recipients.every((r)=>r.state==='SUCCESS')) return 'COMPLETED'; if(!hasSuccess&&hasTerminalFailure&&!hasCancelled) return 'FAILED'; if(batch.cancellationRequested&&!hasProcessing&&!hasRetryWait&&!hasPending) return 'CANCELLED'; if(hasCancelled&&!hasSuccess&&!hasTerminalFailure) return 'CANCELLED'; return batch.state;
  }
  ...
  async confirmBatch(batchId,input){
    await this.ready; const {fingerprint,recipientCount,correlationId,confirmedAt}=input; const batch=this.#batch(batchId); if(batch.state!=='DRAFT') throw new Error(`Batch cannot be confirmed from state ${batch.state}`); if(String(fingerprint)!==batch.preview.fingerprint) throw new Error('Preview fingerprint mismatch'); if(Number(recipientCount)!==batch.preview.recipientCount) throw new Error('Recipient count mismatch');
    batch.state='CONFIRMED'; batch.confirmation={batchId,recipientCount:batch.preview.recipientCount,fingerprint:batch.preview.fingerprint,timestamp:confirmedAt ?? nowIso(this.clock),correlationId:assertString(correlationId ?? batch.correlationId,'correlationId')}; batch.causationId=newId(); batch.updatedAt=nowIso(this.clock); await this.#save(); return batch;
  }
  ...
  async processBatch(batchId){
    await this.ready; const batch=this.#batch(batchId); if(!['QUEUED','PROCESSING'].includes(batch.state)) return batch; if(batch.state==='QUEUED') batch.state='PROCESSING'; batch.updatedAt=nowIso(this.clock); await this.#save(); for(const recipient of Object.values(batch.recipients)){ if(batch.cancellationRequested) break; if(recipient.state==='PENDING'&&!recipient.recoveryBlocked) await this.#processRecipient(batch,recipient); } await this.#updateAggregate(batch); return batch;
  }
  /** @param {string} key @param {TimerHandle} timer */
  #rememberTimer(key,timer){ if(this.retryTimers.has(key)) this.clearTimeoutImpl(this.retryTimers.get(key)); this.retryTimers.set(key,timer); }
  /** @param {string} key */
  #clearTimer(key){ const timer=this.retryTimers.get(key); if(timer!==undefined){ this.clearTimeoutImpl(timer); this.retryTimers.delete(key); } }
  /** @param {DispatchBatch} batch @param {DispatchRecipient} recipient */
  async #processRecipient(batch,recipient){
    if(recipient.state!=='PENDING'||recipient.recoveryBlocked) return; recipient.state='PROCESSING'; const attemptNumber=recipient.attempts.length+1; const attempt=/** @type {DispatchAttempt} */ ({attempt:attemptNumber,idempotencyIdentity:recipient.idempotencyIdentity,startedAt:nowIso(this.clock),phase:'REQUEST_PREPARED',causationId:newId(),effectStatus:'PENDING'}); recipient.attempts.push(attempt); batch.updatedAt=nowIso(this.clock); await this.#save(); attempt.phase='REQUEST_ATTEMPTED'; attempt.requestedAt=nowIso(this.clock); attempt.effectStatus='IN_FLIGHT'; await this.#save();
    const timerKey=`${batch.batchId}:${recipient.identity}:${attemptNumber}`; const timeoutPromise=new Promise((_,reject)=>{ const timer=this.setTimeoutImpl(()=>reject(Object.assign(new Error('PROCESSING timeout exceeded'),{code:'PROCESSING_TIMEOUT'})),PROCESSING_TIMEOUT_MS); this.#rememberTimer(timerKey,timer); });
    try{
      const message=await Promise.race([this.sendText(recipient.identity,recipient.context),timeoutPromise]); this.#clearTimer(timerKey); attempt.phase='PROVIDER_OBSERVED'; attempt.completedAt=nowIso(this.clock); attempt.effectStatus='TRANSPORT_OBSERVED'; attempt.providerObservation={level:'TRANSPORT_OBSERVED',messageId:typeof message==='object'&&message!==null&&'id' in message&&typeof message.id==='string'?message.id:null,deliveryConfirmed:false,readConfirmed:false,...(batch.cancellationRequested?{cancelRequestedBeforeObservation:true}:{})}; recipient.state='SUCCESS'; recipient.nextRetryAt=null; recipient.lastError=null; if(batch.cancellationRequested) batch.cancellationOutcome='CANCELLED_AFTER_PARTIAL_EFFECT';
    }catch(error){
      this.#clearTimer(timerKey);
      if(typeof error==='object'&&error!==null&&'code' in error&&error.code==='PROCESSING_TIMEOUT'){ attempt.phase='UNKNOWN'; attempt.completedAt=nowIso(this.clock); attempt.effectStatus='INDETERMINATE_EFFECT_UNRESOLVED'; attempt.recovery='PROCESSING_TIMEOUT'; attempt.timeoutAt=nowIso(this.clock); recipient.recoveryBlocked=true; recipient.lastError='Processing exceeded the 5-minute recovery threshold; external effect cannot be reconciled safely.'; batch.recoveryRequired=true; }
      else { const failure=classifyFailure(error); attempt.phase='UNKNOWN'; attempt.completedAt=nowIso(this.clock); attempt.failure={class:failure.kind,message:failure.message}; attempt.effectStatus='FAILED_OBSERVED'; recipient.lastError=failure.message; if(failure.kind==='RETRYABLE'&&attemptNumber<MAX_ATTEMPTS&&!batch.cancellationRequested){ const delay=retryDelay(attemptNumber); recipient.state='RETRY_WAIT'; recipient.nextRetryAt=new Date(this.clock()+delay).toISOString(); this.#scheduleRetry(batch.batchId,recipient.identity,delay); } else { recipient.state='FAILED_TERMINAL'; recipient.nextRetryAt=null; } }
    }
    batch.updatedAt=nowIso(this.clock); await this.#save();
  }
  /** @param {string} batchId @param {string} recipientIdentity @param {number} delay */
  #scheduleRetry(batchId,recipientIdentity,delay){ const key=`retry:${batchId}:${recipientIdentity}`; if(this.retryTimers.has(key)) return; const timer=this.setTimeoutImpl(()=>{this.retryTimers.delete(key); void this.retryRecipient(batchId,recipientIdentity).catch((error)=>console.error('[KassisT Dispatch] retry failed:',error instanceof Error?error.message:error));},delay); this.retryTimers.set(key,timer); }
  /** @param {string} batchId @param {string} recipientIdentity */
  async retryRecipient(batchId,recipientIdentity){ await this.ready; const batch=this.#batch(batchId); const recipient=batch.recipients[recipientIdentity]; if(!recipient) throw new Error(`Recipient not found: ${recipientIdentity}`); if(recipient.recoveryBlocked||batch.cancellationRequested||recipient.state!=='RETRY_WAIT') return batch; if(!recipient.nextRetryAt||new Date(recipient.nextRetryAt).getTime()>this.clock()) return batch; if(recipient.attempts.length>=MAX_ATTEMPTS){recipient.state='FAILED_TERMINAL';recipient.nextRetryAt=null;await this.#save();return batch;} recipient.state='PENDING';recipient.nextRetryAt=null;await this.#save();await this.#processRecipient(batch,recipient);await this.#updateAggregate(batch);return batch; }
  /** @param {string} batchId */
  async cancelBatch(batchId){ await this.ready; const batch=this.#batch(batchId); if(['COMPLETED','PARTIAL_FAILURE','FAILED','CANCELLED'].includes(batch.state)) return batch; batch.cancellationRequested=true; for(const recipient of Object.values(batch.recipients)) if(['PENDING','RETRY_WAIT'].includes(recipient.state)) recipient.state='CANCELLED'; if(['DRAFT','CONFIRMED','QUEUED'].includes(batch.state)){batch.state='CANCELLED';batch.cancellationOutcome='CANCELLED_WITHOUT_EFFECT';} else {batch.state=this.#aggregate(batch);if(Object.values(batch.recipients).some((recipient)=>recipient.state==='SUCCESS')) batch.cancellationOutcome='CANCELLED_AFTER_PARTIAL_EFFECT';} batch.causationId=newId(); batch.updatedAt=nowIso(this.clock); await this.#save(); return batch; }
  /** @param {DispatchBatch} batch */
...
export function isDispatchPreview(value){ if(!value||typeof value!=='object') return false; const candidate=/** @type {{status?:unknown,fingerprint?:unknown,recipients?:unknown}} */ (value); return candidate.status==='PREVIEW'&&typeof candidate.fingerprint==='string'&&Array.isArray(candidate.recipients)&&candidate.recipients.every((recipient)=>{ if(!recipient||typeof recipient!=='object') return false; const entry=/** @type {{normalizedNumber?:unknown,context?:unknown,contact?:unknown}} */ (recipient); return typeof entry.normalizedNumber==='string'&&typeof entry.context==='string'&&(entry.contact===undefined||entry.contact===null||typeof entry.contact==='string'); }); }
