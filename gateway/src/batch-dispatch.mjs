...
/** @typedef {{fingerprint:unknown,recipientCount:unknown,correlationId?:string,confirmedAt?:string|undefined}} ConfirmInput */
...
export class BatchDispatchRuntime {
  ...
  async createDraft(preview,{correlationId=newId(),batchId=newId(),createdAt=nowIso(this.clock)}={}){
    ...
    const id=assertString(batchId,'batchId');
    const batches=/** @type {Record<string,DispatchBatch>} */(this.state.batches);
    batches[id]={batchId:id,state:'DRAFT',createdAt,updatedAt:createdAt,correlationId:assertString(correlationId,'correlationId'),causationId:newId(),preview:{fingerprint:String(preview.fingerprint),sourceName:preview.sourceName??'upload.csv',recipientCount:Object.keys(recipients).length,recipients:canonicalRecipients(preview.recipients)},recipients,cancellationRequested:false};
    await this.#save();
  ...
  async confirmBatch(batchId,input){
    ...
    batch.confirmation={batchId:id,recipientCount:batch.preview.recipientCount,fingerprint:batch.preview.fingerprint,timestamp:confirmedAt??nowIso(this.clock),correlationId:assertString(correlationId??batch.correlationId,'correlationId')};
  ...
  async #processRecipient(batch,recipient){
    ...
    const timeoutPromise=new Promise((_,reject)=>{ const timer=this.setTimeoutImpl(()=>reject(Object.assign(new Error('PROCESSING timeout exceeded'),{code:'PROCESSING_TIMEOUT'})),PROCESSING_TIMEOUT_MS); this.#rememberTimer(timerKey,timer); });
    try{
  ...
  #scheduleRetry(batchId,recipientIdentity,delay){
    ...
    const timer=this.setTimeoutImpl(()=>{this.retryTimers.delete(key); void this.retryRecipient(batchId,recipientIdentity).catch((error)=>console.error('[KassisT Dispatch] retry failed:',error instanceof Error?error.message:error));},delay);
  ...
  async cancelBatch(batchId){
    ...
    if(['DRAFT','CONFIRMED','QUEUED'].includes(batch.state)){batch.state='CANCELLED';batch.cancellationOutcome='CANCELLED_WITHOUT_EFFECT';}
    else{batch.state=this.#aggregate(batch);if(Object.values(batch.recipients).some((recipient)=>recipient.state==='SUCCESS'))batch.cancellationOutcome='CANCELLED_AFTER_PARTIAL_EFFECT';}
    batch.causationId=newId();
    batch.updatedAt=nowIso(this.clock);
    await this.#save();
    return batch;
  ...
  async #recoverAfterRestart(){
    ...
    for(const batch of Object.values(batches)){
      if(batch.state==='PROCESSING')for(const recipient of Object.values(batch.recipients)){
