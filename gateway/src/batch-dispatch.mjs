...
function classifyFailure(error) {
  ...
  const statusCode =
    typeof error === 'object' && error !== null && 'output' in error && typeof error.output === 'object' && error.output !== null && 'statusCode' in error.output && typeof error.output.statusCode === 'number'
...
function makeInitialState() {
  ...
  const initial = { version: 1, batches: {} };
  return initial;
}
...
export class BatchDispatchRuntime {
  ...
  constructor(options = {}) {
    ...
    this.clearTimeoutImpl = options.clearTimeoutImpl ?? ((handle) => clearTimeout(/** @type {ReturnType<typeof setTimeout>} */ (handle)));
    /** @type {DispatchJournal} */
  ...
  async queueBatch(batchId, { causationId = newId() } = {}) {
    ...
    batch.updatedAt = nowIso(this.clock);
    await this.#save();
    void this.processBatch(batchId).catch((error) => console.error('[KassisT Dispatch] batch processing failed:', error instanceof Error ? error.message : error));
    return batch;
  }
  ...
  #rememberProcessingTimer(key, timer) {
  ...
  async #processRecipient(batch, recipient) {
    ...
    const timeoutPromise = new Promise<never>((_, reject) => {
      ...
      this.#rememberProcessingTimer(timerKey, timer);
    });
  ...
  async retryRecipient(batchId, recipientIdentity) {
    ...
    if (recipient.recoveryBlocked) return batch;
...
export function isDispatchPreview(value) {
  ...
  return candidate.status === 'PREVIEW'
    ...
    && candidate.recipients.every((recipient) => {
