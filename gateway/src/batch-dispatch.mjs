...
export const RECIPIENT_STATES = Object.freeze([
  'PENDING',
  'PROCESSING',
  'SUCCESS',
  'RETRY_WAIT',
...
function defaultStatePath() {
  return process.env.KASSIST_BATCH_STATE_PATH ?? path.resolve(process.cwd(), '.data/dispatch/batches.json');
}
...
function effectIdentity(batchId, recipientIdentity) {
  return crypto
    .createHash('sha256')
    .update(`${batchId}:${recipientIdentity}`, 'utf8')
    .digest('hex');
}
...
function classifyFailure(error) {
  ...
  const statusCode =
    ...
    typeof error.output === 'object' && error.output !== null &&
  ...
  if (['ECONNRESET', 'ETIMEDOUT', 'ENETUNREACH', 'EAI_AGAIN', 'ECONNREFUSED', 'EPIPE'].some((code) => message.includes(code))) {
    return { kind: 'RETRYABLE', message };
  }
  if (/not connected|connection is closed|temporarily unavailable|timeout/i.test(message)) {
    return { kind: 'RETRYABLE', message };
  }
  if (/required|invalid recipient|malformed|unsupported/i.test(message)) return { kind: 'TERMINAL', message };
...
export class BatchDispatchRuntime {
  constructor({ sendText = defaultSendText, statePath = defaultStatePath(), clock = () => Date.now(), setTimeoutImpl = setTimeout } = {}) {
  ...
  async #load() {
    try {
      ...
      if (!parsed || parsed.version !== 1 || typeof parsed.batches !== 'object') throw new Error('invalid dispatch journal');
      this.state = parsed;
    } catch (error) {
  ...
  #aggregate(batch) {
    ...
    const hasProcessing = recipients.some((recipient) => recipient.state === 'PROCESSING');
    const hasRetryWait = recipients.some((recipient) => recipient.state === 'RETRY_WAIT');
    const hasPending = recipients.some((recipient) => recipient.state === 'PENDING');
    const hasSuccess = recipients.some((recipient) => recipient.state === 'SUCCESS');
    const hasTerminalFailure = recipients.some((recipient) => recipient.state === 'FAILED_TERMINAL');
    const hasCancelled = recipients.some((recipient) => recipient.state === 'CANCELLED');
    ...
    if (recipients.length > 0 && recipients.every((recipient) => recipient.state === 'SUCCESS')) return 'COMPLETED';
  ...
  async queueBatch(batchId, { causationId = newId() } = {}) {
    ...
    void this.processBatch(batchId).catch((error) => {
      console.error('[KassisT Dispatch] batch processing failed:', error instanceof Error ? error.message : error);
    });
    return batch;
  }
  ...
  async processBatch(batchId) {
    ...
    for (const recipient of Object.values(batch.recipients)) {
      ...
      if (recipient.state === 'PENDING') await this.#processRecipient(batch, recipient);
    }
  ...
  async #processRecipient(batch, recipient) {
    if (recipient.state !== 'PENDING') return;
    ...
    try {
      const message = await this.sendText(recipient.identity, recipient.context);
      ...
      if (failure.kind === 'RETRYABLE' && attemptNumber < MAX_ATTEMPTS && !batch.cancellationRequested) {
        ...
        recipient.nextRetryAt = new Date(this.clock() + BACKOFF_MS[attemptNumber - 1]).toISOString();
        this.#scheduleRetry(batch.batchId, recipient.identity, BACKOFF_MS[attemptNumber - 1]);
      } else {
  ...
  #scheduleRetry(batchId, recipientIdentity, delay) {
    const key = `${batchId}:${recipientIdentity}`;
    if (this.retryTimers.has(key)) return;
    const timer = this.setTimeoutImpl(() => {
      this.retryTimers.delete(key);
      void this.retryRecipient(batchId, recipientIdentity).catch((error) => {
        console.error('[KassisT Dispatch] retry failed:', error instanceof Error ? error.message : error);
      });
    }, delay);
  ...
  async retryRecipient(batchId, recipientIdentity) {
    await this.ready;
    const batch = this.#batch(batchId);
    const recipient = batch.recipients[recipientIdentity];
    if (!recipient) throw new Error(`Recipient not found: ${recipientIdentity}`);
    if (batch.cancellationRequested || recipient.state !== 'RETRY_WAIT') return batch;
    if (!recipient.nextRetryAt || new Date(recipient.nextRetryAt).getTime() > this.clock()) {
      return batch;
    }
  ...
  async #updateAggregate(batch) {
    ...
    else if (next !== 'CONFIRMED' && next !== 'QUEUED' && next !== 'DRAFT') batch.state = next;
    batch.updatedAt = nowIso(this.clock);
    await this.#save();
  }
  ...
  async #recoverAfterRestart() {
    for (const batch of Object.values(this.state.batches)) {
      if (batch.state === 'PROCESSING') {
        for (const recipient of Object.values(batch.recipients)) {
          const currentAttempt = recipient.attempts.at(-1);
      ...
      for (const recipient of Object.values(batch.recipients)) {
        if (recipient.state === 'RETRY_WAIT' && recipient.nextRetryAt) {
